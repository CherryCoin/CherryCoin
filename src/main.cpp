...
const string strMessageMagic = "Cherrycoin Signed Message:\n";
...
{
    // Cherrycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every million blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 1000000); // Cherrycoin: 1 million blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Cherrycoin: Kimoto took over
static const int64 nTargetSpacing = 60; // Cherrycoin: 1 minute
...
// Kimoto's gravity well implementation.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cherrycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Cherrycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cherrycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CherrycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CherrycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CherrycoinMiner(CWallet *pwallet)
{
    printf("CherrycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cherrycoin-miner");
    ...
    try { loop {
        ...
        printf("Running CherrycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CherrycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CherrycoinMiner, pwallet));
}
